...
import type { EDL, JobMeta, SourceEDLSegment, VideoFile } from '@/types/edl'
...
export async function listFiles(folder: string): Promise<VideoFile[]> {
  const delays = [0, 180, 700]
  for (let i = 0; i < delays.length; i++) {
    if (delays[i] > 0) await new Promise((resolve) => window.setTimeout(resolve, delays[i]))
    try {
      ...
      if (!res.ok) continue
      const files = (await res.json()) as VideoFile[]
      if (files.length > 0 || i === delays.length - 1) return files
    } catch {
      // Packaged startup can briefly race the backend. Retry before showing
...
export class JobSaveConflictError extends Error {
  job_rev?: string | null

  constructor(jobRev?: string | null) {
    super('job.yaml changed on disk')
    this.name = 'JobSaveConflictError'
    this.job_rev = jobRev
  }
}

export async function saveJob(folder: string, meta: JobMeta): Promise<string | null> {
  ...
  if (res.status === 409) {
    let jobRev: string | null = null
    try {
      const data = await res.json()
      const detail = data && typeof data === 'object' ? (data as { detail?: unknown }).detail : null
      if (detail && typeof detail === 'object') {
        const rawRev = (detail as { job_rev?: unknown }).job_rev
        jobRev = typeof rawRev === 'string' ? rawRev : null
      }
    } catch {
      jobRev = null
    }
    throw new JobSaveConflictError(jobRev)
  }
  if (!res.ok) throw new Error(`Failed to save job.yaml (${res.status})`)
  const data = (await res.json()) as { ok?: boolean; job_rev?: string | null }
  return typeof data.job_rev === 'string' ? data.job_rev : null
}
...
function normalizeSourceEDL(raw: unknown): SourceEDLSegment[] {
  if (!raw || typeof raw !== 'object') return []
  const obj = raw as { segments?: unknown }
  if (!Array.isArray(obj.segments)) return []
  return obj.segments
    .map((item): SourceEDLSegment | null => {
      if (!item || typeof item !== 'object') return null
      const s = item as Record<string, unknown>
      const file = typeof s.file === 'string' ? s.file : ''
      const start = asNumber(s.start)
      const end = asNumber(s.end)
      if (!file || end <= start) return null
      return {
        file,
        start,
        end,
        label: typeof s.label === 'string' && s.label ? s.label : 'SOURCE',
        enabled: s.enabled !== false,
        group_id: typeof s.group_id === 'string' ? s.group_id : undefined,
      } satisfies SourceEDLSegment
    })
    .filter((s): s is SourceEDLSegment => s !== null)
}

export async function loadSourceEDL(folder: string): Promise<SourceEDLSegment[]> {
  const res = await fetch(`/api/source-edl?folder=${encodeURIComponent(folder)}`)
  if (!res.ok) return []
  return normalizeSourceEDL(await res.json())
}

export async function saveSourceEDL(folder: string, segments: SourceEDLSegment[]): Promise<SourceEDLSegment[]> {
  const clean = segments.filter((s) => s.file && s.end > s.start)
  if (clean.length === 0) {
    await deleteSourceEDL(folder)
    return []
  }
  const res = await fetch('/api/source-edl', {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ folder, source_edl: { segments: clean } }),
  })
  if (!res.ok) throw new Error(`failed to save source ranges (${res.status})`)
  return normalizeSourceEDL(await res.json())
}

export async function deleteSourceEDL(folder: string): Promise<void> {
  await fetch(`/api/source-edl?folder=${encodeURIComponent(folder)}`, { method: 'DELETE' })
}
...
export function rawVideoUrl(file: string): string {
  return `/api/video_stream?path=${encodeURIComponent(file)}`
}
