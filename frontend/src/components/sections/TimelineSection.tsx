...
import { Award, Plus, Scissors, Trash2 } from 'lucide-react'
...
export default function TimelineSection({
  ...
  selectedLaneFile = null, selectedLaneFiles = [], importSelectedFiles = [], onSelectLaneFile,
  onReorderLane,
  playheadLaneFile = null,
  introMarker = null, introSelected = false,
  onSelectIntro, onDeleteIntro, onUnsetIntro, canSetIntro = false, onSetSelectedAsIntro,
  onIntroResize, onIntroMove,
}: Props) {
  // EDL 鐜板湪鐢ㄧ粺涓€ segments 鍒楄〃;body = 闈?INTRO 鏍囩閭ｆ壒銆?
  ...
  const canSetOrUnsetIntro = introSelected || canSetIntro
  ...
  const deleteSelected = () => {
    if (introSelected) {
      onDeleteIntro?.()
      if (selectedIdxSet.size === 0 && selectedIdx === null) onSelectIdx(null)
      return
    }
    ...
    if (laneDoomed.size > 0) {
      ...
      // lane (FilePlus 鈫?tick) brings the windows back automatically.
  ...
  const handleAddSegment = (file: string, start: number, end: number) => {
    ...
    // Sort by visible lane order, then by start within file.
    const fileOrder = new Map<string, number>()
    files.forEach((f, i) => {
      fileOrder.set(f.path, i)
      fileOrder.set(f.name, i)
    })
    if (!fileOrder.has(file)) fileOrder.set(file, segments.length)
    next.sort((a, b) => {
      const fa = fileOrder.get(a.file) ?? fileOrder.get(a.file.split(/[\\/]/).pop() ?? a.file) ?? Number.MAX_SAFE_INTEGER
      const fb = fileOrder.get(b.file) ?? fileOrder.get(b.file.split(/[\\/]/).pop() ?? b.file) ?? Number.MAX_SAFE_INTEGER
    ...
    <div className="panel h-full">
      ...
      >
        <TimelineToolbar
          ...
          canRedo={canRedo}
          canDelete={canDelete}
          canSetIntro={canSetOrUnsetIntro}
          ...
          onUndo={onUndo}
          onRedo={onRedo}
          onSetSelectedAsIntro={introSelected ? onUnsetIntro : onSetSelectedAsIntro}
      ...
      {/* Timeline body 鈥?dimmed + non-interactive while addingFiles is on
        ...
        {menu && createPortal((
          ...
          >
            {[
              ...
              {
                label: introSelected ? 'Unset intro' : 'Set intro',
                icon: <Award size={13} />,
                action: introSelected ? onUnsetIntro : onSetSelectedAsIntro,
                disabled: introSelected ? !onUnsetIntro : (!canSetIntro || !onSetSelectedAsIntro),
              },
              { sep: true },
      ...
      >
        ...
        {mode !== 'pointer' ? (
          ...
          <span style={{ color: 'rgb(var(--text-muted))', marginLeft: 'auto' }}>
            drag selected edge resize | drag playhead seek | <kbd>Space</kbd> play/pause | <kbd>Alt</kbd>+wheel zoom | middle-drag pan | <kbd>Shift+A</kbd> add | <kbd>Shift+S</kbd> split | <kbd>Shift+W</kbd> window | <kbd>Del</kbd> delete
          </span>
        )}
        {/* Logs popup button removed 鈥?pipeline panel now has an inline
