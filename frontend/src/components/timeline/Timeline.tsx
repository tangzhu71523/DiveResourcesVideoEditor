...
interface Props {
  ...
  // PreviewBox playback state — playhead is locked while playing.
  isPlaying?: boolean
...
export default function Timeline({
  ...
  selectedLaneFile, selectedLaneFiles = [], onSelectLaneFile,
  ...
  // Track Shift key globally so trim handles can switch hit-area on/off in
  // real time. Without Shift the handles are pointer-events:none and clicks
  // fall through to the playhead drag layer behind them.
  ...
  // Middle-click pan scope. Narrowed to the cover-bar + video-track area
  // only — black background above/below the lanes is excluded so middle
  // clicks there don't grab the timeline. When `segments.length === 0`
  // this ref never attaches → middle clicks fall through to a no-op
  // swallower on `containerRef` and never bubble up to App-level pan.
  ...
  const lanes: FileLane[] = useMemo(() => {
    ...
    segments.forEach((seg, i) => {
      const arr = segsByFile.get(seg.file) ?? []
      arr.push({ seg, globalIdx: i })
      segsByFile.set(seg.file, arr)
    })
    ...
    files.forEach((f, idx) => {
      const segs = (segsByFile.get(f.path) ?? []).slice().sort((a, b) => a.seg.start - b.seg.start)
      out.push({
        file: f.path,
  ...
  const firstBodyGlobalStart = useMemo(() => {
    if (liveLanes.length === 0) return 0
    for (const lane of liveLanes) {
      if (lane.segments.length > 0) {
        const sortedStart = lane.segments.reduce(
          (m, { seg }) => Math.min(m, seg.start),
          lane.segments[0].seg.start,
        )
        return lane.offsetSec + sortedStart
      }
    }
    return 0
  }, [liveLanes])
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      if (s) {
        ...
        const worldSec = (s.scrollLeft + xInView) / pxPerSec
  ...
  useEffect(() => {
    ...
    if (a) { worldSec = a.worldSec; xInView = a.xInView; zoomAnchorRef.current = null }
      ...
      worldSec = (s.scrollLeft + xInView) / prevPxPerSec
    }
    s.scrollLeft = Math.max(0, worldSec * newPxPerSec - xInView)
    prevZoomRef.current = zoom
  }, [zoom, pxPerSec, totalExpandedSec, totalGapPx, viewportWidth])
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      const s = scrollRef.current
      const anchorSec = target === 'title' ? 0 : firstBodyGlobalStart
      const anchorPx = anchorSec * pxPerSec
      if (s) {
  ...
  const handlePlayheadMouseDown = useCallback((e: React.MouseEvent) => {
    // Playback wins: while the PreviewBox video is playing, the playhead
    // is locked (only the video can advance it). Drag is rejected so the
    // user has to pause first to take control back.
    if (isPlaying) return
  ...
  }, [renderedPlayheadPx, pxPerSec, lanes, onSeek, totalWidth, isPlaying])
  ...
  const handleWindowSplitClick = useCallback((e: React.MouseEvent, segIdx: number) => {
    ...
    const lane = laneByFile.get(seg.file); if (!lane) return
    const rs = clientXToRailSec(e.clientX)
    const src = rs - lane.offsetSec
    if (src <= seg.start + 0.5 || src >= seg.end - 0.5) return
    onSplitSegment(segIdx, src)
    onModeChange?.('pointer')
  }, [mode, onSplitSegment, onModeChange, segments, laneByFile, clientXToRailSec])
  ...
  if (lanes.length === 0) {
    ...
    return (
      <div ref={containerRef} className="relative h-full w-full flex flex-col overflow-hidden" style={{ background: 'var(--track-bg, #2a2a2a)' }}>
    ...
    <div ref={containerRef} className="relative h-full w-full flex flex-col overflow-hidden" style={{ background: 'var(--track-bg, #2a2a2a)' }}>
      ...
      >
        ...
        >
            ...
            {lanes.map((lane) => {
              ...
              <div
                ...
                style={{
                  ...
                  padding: '0 6px',
                  overflow: 'hidden',
                  cursor: onSelectLaneFile ? 'pointer' : 'default',
                  background: laneSelected ? 'rgba(29,78,216,0.35)' : 'transparent',
                  borderRadius: 3,
          ...
          <div className="relative select-none" style={{ height: 10, flexShrink: 0 }}>
            {renderedPlayheadPx !== null && (
              <div
                ...
                style={{ left: renderedPlayheadPx, transform: 'translateX(-50%)' }}
                onMouseDown={handlePlayheadMouseDown}
                title="Drag to seek · sets next play start"
              >
                <div
                  style={{
                    width: 0,
          ...
          {/* Pan scope — middle-click pan only engages on the video track. */}
          ...
          >
            ...
            {lanes.map((lane, li) => (
              ...
              >
                <FilmStrip hue={hueFor(lane.file)} fogged filePath={lane.file} durationSec={lane.duration} widthPx={Math.round(lane.duration * pxPerSec)} folder={folder} />
              </div>
            ))}
            ...
            {/* Split cursor */}
            {mode === 'split' && <SplitCursor containerRef={containerRef} scrollRef={scrollRef} />}
...
function SplitCursor({
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      ...
      const rect = s.getBoundingClientRect()
      setXPx(e.clientX - rect.left + s.scrollLeft)
    }
...
function SegmentWindow({
  ...
  minStart, maxEnd, mode, onSelect, onSplitClick, onResize, onMove,
  onLiveChange, onLiveEnd, folder, shiftHeld = false,
}: WindowProps) {
  ...
  const trimHoverActive = mode === 'pointer' && selected && shiftHeld && hoverEdge !== null
  const themeEdgeColor = 'var(--track-border, #e8e6de)'
  const outlineColor = isIntro ? ACCENT : themeEdgeColor
  const windowOutline = (!selected && isIntro)
    ? `1px solid ${outlineColor}`
    : 'none'
  const windowShadow = selected
    ? '0 12px 26px rgba(0,0,0,0.38), 0 3px 8px rgba(0,0,0,0.28)'
    : isIntro
  ...
  const handleResizeMouseDown = useCallback(
    ...
    [pxPerSec, seg.start, seg.end, globalIdx, onResize, minStart, maxEnd, left, mode, onLiveChange, onLiveEnd, selected],
  )
  ...
  return (
    <div
      ...
      style={{
        ...
        transform: selected ? 'translateY(-2px) scale(1.012)' : 'none',
        transformOrigin: 'center center',
        transition: dragMode ? 'none' : 'transform 120ms ease, filter 120ms ease',
      }}
      ...
      onMouseMove={(e) => {
        ...
        if (mode === 'pointer' && selected) {
          ...
          el.style.cursor = shiftHeld && nextEdge ? 'ew-resize' : 'default'
          return
        }
      ...
      title={mode === 'split' ? 'Click to split here' : 'Click to select - Shift+drag edge to resize - Delete to remove'}
    >
      <div
        style={{
      ...
      >
        ...
        {!compact && (
          <>
            ...
            >
              <FilmStrip hue={hueFor(laneFile)} fogged={false} filePath={laneFile} durationSec={laneDuration} widthPx={Math.round(laneWidthPx)} folder={folder} />
            </div>
            <div
              className="absolute inset-0 pointer-events-none"
        ...
        <>
          <div
            ...
            style={{
              ...
              background: themeEdgeColor,
              borderRadius: 3,
              boxShadow: '0 0 0 1px rgba(0,0,0,0.28), 0 8px 18px rgba(0,0,0,0.24)',
            }}
          />
          <div
            className="absolute pointer-events-none z-30"
        ...
        <>
          ...
          <div
            ...
            style={{
              ...
              cursor: (selected && shiftHeld) ? 'ew-resize' : 'default',
              ...
              pointerEvents: (selected && shiftHeld) ? 'auto' : 'none',
              background: 'transparent',
            }}
            onMouseDown={(e) => handleResizeMouseDown(e, 'start')}
            title={selected ? 'Shift+drag to adjust start' : 'Click window to select first'}
          />
          ...
          <div
            ...
            title={selected ? 'Shift+drag to adjust end' : 'Click window to select first'}
          />
        </>
      )}
    </div>
