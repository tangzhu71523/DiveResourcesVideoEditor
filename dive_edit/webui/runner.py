...
def _load_runner_timeouts(root: Path) -> tuple[float, float]:
    """Read WebUI subprocess watchdog limits from config.yaml."""
    idle_timeout_sec = 600.0
    max_runtime_sec = 8 * 60 * 60.0
    cfg_path = root / "config.yaml"
    try:
        data = yaml.safe_load(cfg_path.read_text(encoding="utf-8")) or {}
    except (OSError, yaml.YAMLError):
        data = {}
    runner_cfg = data.get("runner", {}) if isinstance(data, dict) else {}
    if not isinstance(runner_cfg, dict):
        runner_cfg = {}
    try:
        idle_timeout_sec = max(
            30.0,
            float(runner_cfg.get("idle_timeout_sec", idle_timeout_sec)),
        )
    ...
    try:
        max_runtime_sec = max(
            idle_timeout_sec,
            float(runner_cfg.get("max_runtime_sec", max_runtime_sec)),
        )
    except (TypeError, ValueError):
        pass
    return idle_timeout_sec, max_runtime_sec
...
class RunManager:
    ...
    def start(
        ...
        idle_timeout_sec, max_runtime_sec = _load_runner_timeouts(Path(cwd))
        ...
        threading.Thread(
            target=self._watchdog, args=(job,), daemon=True, name=f"watchdog-{job_id}"
        ).start()
        ...
        try:
            ...
            job.loop.call_soon_threadsafe(
                ...
                Event(
                    ...
                    {
                        "exit_code": exit_code,
                        **({"msg": job.timed_out_reason} if job.timed_out_reason else {}),
                    },
                ),
            )

    def _watchdog(self, job: RunJob, check_interval_sec: float = 2.0) -> None:
        """Kill a stuck pipeline subprocess and surface a readable error."""
        while job.proc.poll() is None:
            now = time.time()
            runtime_sec = now - job.started_at
            idle_sec = now - job.last_output_at
            reason: str | None = None
            if runtime_sec >= job.max_runtime_sec:
                reason = (
                    f"Pipeline timeout: exceeded max runtime "
                    f"{job.max_runtime_sec:.0f}s without finishing."
                )
            elif idle_sec >= job.idle_timeout_sec:
                reason = (
                    f"Pipeline timeout: no output for {job.idle_timeout_sec:.0f}s. "
                    "The process was stopped to prevent the app from hanging."
                )
            if reason:
                job.timed_out_reason = reason
                job.loop.call_soon_threadsafe(
                    self._enqueue,
                    job,
                    Event("log", {"msg": f"ERROR {reason}"}),
                )
