...
from ..utils.gpu import detect_optimal_workers, select_pipeline_workers
from ..utils.paths import app_root, is_frozen
...
DEFAULT_INITIAL_DIR = r"C:\Users\wlzhe\DIVE RESOURCES\VIDEO"
...
def api_list_files(payload: FolderPayload) -> list[VideoFileDto]:
    ...
    out: list[VideoFileDto] = []
    for path, dur in list_with_durations(folder):
        try:
...
def api_get_job(folder: str = Query(...)) -> JobMetaDto | None:
    ...
    if meta is None:
        return None
    return JobMetaDto(**meta.to_dict())


@app.put("/api/job")
def api_put_job(payload: SaveJobRequest) -> dict[str, bool]:
...
def api_put_edl(payload: SaveEDLRequest) -> dict[str, bool]:
    """Write user edits to the DRAFT file (not the official _edl.json).
    ...
    try:
        _edl_draft_path(p).write_text(
            json.dumps(data, indent=2, ensure_ascii=False), encoding="utf-8"
        )
    except OSError as e:
        raise HTTPException(status_code=500, detail=f"failed to write _edl.draft.json: {e}")
    return {"ok": True}
...
def api_delete_job_cache(folder: str = Query(...)) -> dict[str, Any]:
    ...
    Removes the entire <job>/_diveedit/ tree EXCEPT job.yaml.
    """
    import shutil
...
async def api_run(payload: RunRequest) -> RunResponse:
    ...
    workers, _worker_msg = select_pipeline_workers(env=os.environ)
