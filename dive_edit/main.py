...
def run_pipeline(
    *,
    ...
    paths = ensure_job_subdirs(job_folder, output_dir=explicit_output_dir)
    ...
    if args.render_only and edl_path.exists():
        ...
        if meta.body_files:
            ...
            if not analyze_targets:
                ...
                return 3
            body_whitelist: set[str] | None = {name for name in allowed}
        else:
        ...
        if meta.intro_file:
            candidate = job_folder / meta.intro_file
            if candidate.exists():
        ...
        if intro_path is None:
            ...
            if detection.score < min_score:
                ...
                logger.warn(
                    f"  ⚠️ INTRO soft-fallback selected: {intro_path.name} "
                    f"(score={detection.score} < min_score={min_score})"
                )
                logger.warn(
                    f"  ⚠️ Please verify manually; on mistake set intro_file in job.yaml then --reset."
                )
            else:
                logger.info(
            ...
            meta.intro_file = intro_path.name
        ...
        for f in analyze_targets:
            try:
                mt = _dt.fromtimestamp(_os.path.getmtime(f))
                mtime_map[f] = mt
            except OSError:
                mt = None
        ...
        if missing:
            ...
            for f in missing:
                try:
                    ...
                    fts = extract_file_timestamps(f, dur, timestamp_crop=ts_crop_cfg) if dur > 0 else None
                except Exception as exc:  # noqa: BLE001
                    logger.warn(f"  [OCR anchor] {f.name}: failed ({exc})")
                    fts = None
                if fts is not None and fts.first_time is not None:
                    ...
                    logger.info(
                        f"  [OCR anchor] {f.name}: {fts.first_time.strftime(fmt)} "
        ...
        if intro_time is not None:
            ...
            intro_idx = next(
                (i for i, f in enumerate(analyze_targets) if f == intro_path),
                -1,
            )
            body_files = [
                f for i, f in enumerate(analyze_targets)
        ...
        # positive — real dive reports produce ≥20 min of content).
        ...
        edl = build_edl(
            ...
            speech_lock_cfg=config.get("speech_lock", {}),
