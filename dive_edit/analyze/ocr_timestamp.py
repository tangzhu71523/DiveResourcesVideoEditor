...
def extract_file_timestamps(
    ...
    """Extract timestamps via consistency voting across multiple sampled frames.

    Strategy:
      1. Sample OCR at multiple offsets (e.g. 0.5s, 10%, 25%, 50%, 75%, 90%, end).
      2. For each pair of readings, check if their time difference matches
         their offset difference. OCR errors are inconsistent outliers.
      3. Find the LARGEST consistent subset — this is the ground truth.
      4. Compute file start time as the median of (reading_time - offset)
         across the consistent set.
      5. If one reading lacks date (only HH:MM:SS), borrow the date from
         another consistent reading that has it.
    """
    if timestamp_crop is None:
        timestamp_crop = {"x": 1200, "y": 40, "w": 720, "h": 60}

    # Sample 5 positions — odd count avoids 50/50 voting ties.
    raw_offsets = [
        0.5,
        duration_sec * 0.25,
        duration_sec * 0.50,
        duration_sec * 0.75,
        max(0.5, duration_sec - 1.0),
    ]
    offsets = sorted(set(
    ...
    consistent = _find_consistent_samples(samples)

    # Borrow date from any consistent sample that has one
    _HAS_DATE = lambda t: t.year > 2000
    date_anchor = next((t for _, t, _, _ in consistent if _HAS_DATE(t)), None)
    if date_anchor is not None:
        fixed: list[tuple[float, datetime, str, float]] = []
        for off, t, txt, conf in consistent:
            if not _HAS_DATE(t):
                t = t.replace(
    ...
    # Compute start time from consistent samples (median vote)
    first_time = _start_time_from_samples(consistent)
    last_time = (
        first_time + timedelta(seconds=duration_sec)
        if first_time is not None else None
    )

    # Pick representative texts (smallest offset = "first", largest = "last")
    first_sample = min(consistent, key=lambda s: s[0]) if consistent else None
    last_sample = max(consistent, key=lambda s: s[0]) if consistent else None
